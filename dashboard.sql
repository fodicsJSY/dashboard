...
FROM (
    SELECT
        (SELECT SUM(FACE_YOUNGMALE + FACE_ADULTMALE + FACE_MIDDLEMALE + FACE_SENIORMALE) FROM TB_AI_NVR_DASH_BOARD_DAILY_COUNT_SUMMARY WHERE OCCU_DATE = '20231110') AS total_male_faces,
        (SELECT SUM(FACE_NOMASKMALEYOUNG + FACE_NOMASKMALEADULT + FACE_NOMASKMALEMIDDLE + FACE_NOMASKMALESENIOR) FROM TB_AI_NVR_DASH_BOARD_DAILY_COUNT_SUMMARY WHERE OCCU_DATE = '20231110') AS nomask_male_faces
) AS subquery;
